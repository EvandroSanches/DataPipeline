...
class Twitter_Hook(HttpHook):
    ...
    def create_url(self):
        ...
        url = (f"{self.base_url}/2/tweets/search/recent?query={query}&{tweet_fields}"
               f"&{user_fields}{start_time}{end_time}")
        return url

    def connect_to_endpoint(self, url, session):
        response = requests.Request("GET", url, session)
...
if __name__ == "__main__":
    for pg in Twitter_Hook("AluraOnline").run():
